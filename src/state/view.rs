...
#[repr(u32)]
pub enum Tonemap {
    /// Raw clamped passthrough — no display encoding at all.
    Linear = 0,
    Reinhard = 1,
    Aces = 2,
    Filmic = 3,
    Gamma = 4,
    /// John Hable's piecewise power-curve filmic tonemap. See
    /// filmicworlds.com/blog/filmic-tonemapping-with-piecewise-power-curves.
    Hable = 5,
    /// sRGB display encoding (no tone curve — just clamp + sRGB OETF).
    Srgb = 6,
}
...
impl Tonemap {
    ...
    pub fn label(self) -> &'static str {
        match self {
            ...
            Self::Aces => "ACES",
            Self::Filmic => "Filmic",
            Self::Hable => "Piecewise",
            Self::Gamma => "Gamma",
        }
    }
}

/// Per-pixel diff visualization modes (mirrors Framewise's `ErrorMetric`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(u32)]
pub enum DiffMode {
    Abs = 0,
...
pub struct DisplayUniform {
    ...
    /// xy = scale, zw = offset.
    pub uv_xform: [f32; 4],
    /// Piecewise tonemap (toe_x, toe_y, shoulder_x, shoulder_y).
    pub piecewise: [f32; 4],
    /// (piecewise_white, reinhard_white, gamma, _pad)
    pub tm_extras: [f32; 4],
    pub exposure: f32,
    pub tonemap: u32,
    pub channel: u32,
...
impl Default for DisplayUniform {
    fn default() -> Self {
        Self {
            ...
            piecewise: [0.25, 0.18, 0.78, 0.69],
            tm_extras: [12.0, 4.0, 2.2, 0.0],
            exposure: 0.0,
            tonemap: Tonemap::Srgb as u32,
